...
oknodowpisywania.Size = UDim2.new(1, 0, 0.8, 0) -- Zmniejszamy wysokość okna wpisywania
...
dragButton.Size = UDim2.new(1, 0, 0.2, 0) -- Przycisk zajmuje dolną część ramki
...
if ScreenGui and game.Players.LocalPlayer:WaitForChild("PlayerGui") then
...
w1:Button(
    "         ROZPIERDOL",
    function()
        trackPlayer(oknodowpisywania.Text) -- Zmieniamy textBox na oknodowpisywania tutaj.
    end
)
...
function trackPlayer(playerName)
    ...
    -- Dodajemy zmienną do przechowywania czasu rozpoczęcia śledzenia.
    local startTime = tick()
    
    local c = game:GetService("RunService").Heartbeat:Connect(function()

        -- Sprawdzamy, czy upłynęło już 15 sekund.
        if tick() - startTime >= 15 then
            -- Jeśli tak, zakończamy śledzenie i odłączamy funkcję.
        ...
        if c ~= nil then -- Dodane sprawdzenie, czy 'c' nie jest 'nil'
            c:Disconnect()
        end
...
-- PROBLEM W KONSOLI = :139: attempt to index nil with 'Disconnect' Stack Begin Scrit '', Line 139 Stack End